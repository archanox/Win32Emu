...
public class NeImageLoaderTests
{
	...
	[Fact]
	public void LoadFromBytes_WithAlternativeOffsetInterpretation_ParsesModulesCorrectly()
	{
		// Arrange
		var vm = new VirtualMemory(256 * 1024 * 1024, NullLogger.Instance); // 256MB
		var loader = new NeImageLoader(vm, NullLogger.Instance);
		...
		// instead of imported names table (tests fallback logic)
		var neData = CreateNEFileWithAlternativeOffsets();

		// Act - Should use fallback logic to parse modules correctly
		var image = loader.LoadFromBytes(neData, "<test>");

		// Assert - Should load successfully with fallback offset interpretation
		Assert.NotNull(image);
		Assert.Equal(0x00010000u, image.BaseAddress);
	}
	...
	/// instead of imported names table. This tests the fallback offset interpretation logic.
	/// </summary>
	private static byte[] CreateNEFileWithAlternativeOffsets()
	{
		var data = new byte[2048];
		...
		// Module reference count: 2 modules
		WriteUInt16(data, neOffset + 0x20, 2);
		...
		WriteUInt16(data, neOffset + 0x28, 0x50); // Resident name table
		WriteUInt16(data, neOffset + 0x2A, 0x60); // Module reference table
		WriteUInt16(data, neOffset + 0x2C, 0xA0); // Imported names table
		WriteUInt32(data, neOffset + 44, 0);
		WriteUInt16(data, neOffset + 0x32, 0);
		WriteUInt16(data, neOffset + 0x34, 4);
		data[neOffset + 0x38] = 2;
		...
		// Imported names table at neOffset + 0xA0 (absolute offset 0x120)
		// "KERNEL" at offset 0xA0
		data[neOffset + 0xA0] = 6; // Length
		data[neOffset + 0xA1] = (byte)'K';
		data[neOffset + 0xA2] = (byte)'E';
		data[neOffset + 0xA3] = (byte)'R';
		...
		// Module reference table at neOffset + 0x60
		// IMPORTANT: These offsets are relative to NE header (0x80), not imported names table!
		// This simulates the alternative offset interpretation that some NE files use.
		WriteUInt16(data, neOffset + 0x60, 0xA0); // Offset 0xA0 from NE header = "KERNEL"
		WriteUInt16(data, neOffset + 0x62, 0xA7); // Offset 0xA7 from NE header = "USER"
