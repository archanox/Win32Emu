...
namespace Win32Emu.Win32.Modules
{
	public class DDrawModule : IWin32ModuleAsync
	{
		...
		private uint ComQueryInterface(ICpu cpu, VirtualMemory memory)
		{
			...
			// Read the 16-byte GUID from memory
			var guidBytes = new byte[16];
			for (int i = 0; i < 16; i++)
			{
				guidBytes[i] = _env.MemRead8(riid + (uint)i);
			}
			var requestedIid = new Guid(guidBytes);

			_logger.LogInformation("[DDraw COM] IUnknown::QueryInterface(this=0x{ThisPtr:X8}, riid={Iid}, ppvObject=0x{PpvObject:X8})",
			...
			// Per COM spec: always write null to ppvObject on failure
			_env.MemWrite32(ppvObject, 0);
			_logger.LogWarning("[DDraw COM] QueryInterface returning E_NOINTERFACE for unknown IID {Iid}", requestedIid);
