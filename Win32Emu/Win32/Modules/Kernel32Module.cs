...
internal class Kernel32Module : IWin32ModuleUnsafe
{
	...
	// Note: KnownSystemDlls is now auto-generated by SystemDllsGenerator from IWin32ModuleUnsafe implementations
	...
	private uint LoadLibraryA(in LpcStr lpLibFileName)
	{
		...
		if (!KnownSystemDlls.DllNames.Contains(normalizedName))
		{
	...
	public uint GetCommandLineA()
	{
		...
		if (ptr != 0)
		{
			...
			if (windowsPath != cmdLine)
			{
				...
				// Ensure proper null termination (single null terminator, as on Windows)
				_env.MemWriteBytes(ptr + (uint)bytes.Length, [0]);
	...
	private uint VirtualAlloc(uint lpAddress, uint dwSize, uint flAllocationType, uint flProtect)
	{
		...
		_env.LastError = (uint)(addr != 0
			? NativeTypes.Win32Error.ERROR_SUCCESS
			: NativeTypes.Win32Error.ERROR_NOT_ENOUGH_MEMORY);
	...
	private uint WinExec(in LpcStr lpCmdLine, uint uCmdShow)
	{
		...
		// Resolve the executable path relative to current directory using shared method
		var resolvedPath = _env.ResolveExecutablePath(executable, _env.CurrentDirectory);
