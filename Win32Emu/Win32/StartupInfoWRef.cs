...
{
	/// <summary>
	/// Ref struct wrapper for STARTUPINFOW that provides direct memory access via properties.
	/// Properties automatically read from and write to the underlying memory address.
	/// </summary>
	public readonly ref struct StartupInfoWRef
	{
		private readonly VirtualMemory _memory;
		private readonly uint _address;

		public StartupInfoWRef(VirtualMemory memory, uint address)
		{
			_memory = memory;
			_address = address;
		}
		...
		/// </summary>
		public NativeTypes.STARTUPINFOW ToStruct()
		{
			return new NativeTypes.STARTUPINFOW
			{
				cb = cb,
		...
		/// </summary>
		public static implicit operator NativeTypes.STARTUPINFOW(StartupInfoWRef refStruct) => refStruct.ToStruct();
	}
}
