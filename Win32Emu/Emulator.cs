        ...
        while (!_stopRequested && !_env!.ExitRequested)
        {
            ...
            try
            {
                ...
                if (step.IsCall && step.CallTarget >= 0x0F000000 && step.CallTarget < 0x10000000)
                {
                    ...
                    var alignedTarget = step.CallTarget & IMPORT_STUB_ALIGNMENT_MASK;
                    
                    if (currentImage.ImportAddressMap.TryGetValue(alignedTarget, out var imp))
                    {
                        var dll = imp.dll.ToUpperInvariant();
                        var name = imp.name;
                        _logger.LogInformation("[Import] Direct call to {Dll}!{Name} at 0x{CallTarget:X8}", dll, name, step.CallTarget);
                ...
                if (step.IsSyscall)
                {
                    ...
                    {
                        _logger.LogError("[Syscall] Import stub at 0x{StubAddr:X8} not found in ImportAddressMap", importStubAddr);
                        _cpu.SetRegister("EAX", 0);
                    }
