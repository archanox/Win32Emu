...
public class ProcessEnvironment
{
    ...
    public ProcessEnvironment(VirtualMemory vm, uint heapBase = 0x01000000)
    { _vm = vm; _allocPtr = heapBase; }
    ...
    { if (size == 0) size = 1; var addr = _allocPtr; _allocPtr = AlignUp(_allocPtr + size, 16); return addr; }

    public void RequestExit() => _exitRequested = true;

    // Guest memory helpers
    public uint WriteAnsiString(string s)
    { var bytes = System.Text.Encoding.ASCII.GetBytes(s); var addr = SimpleAlloc((uint)bytes.Length); _vm.WriteBytes(addr, bytes); return addr; }
    public void WriteAnsiStringAt(uint addr, string s, bool nullTerminate = true)
    { var bytes = System.Text.Encoding.ASCII.GetBytes(nullTerminate ? s + "\0" : s); _vm.WriteBytes(addr, bytes); }
    public string ReadAnsiString(uint addr)
    { var buf = new List<byte>(); var p = addr; for(;;){ var b=_vm.Read8(p++); if (b==0) break; buf.Add(b);} return System.Text.Encoding.ASCII.GetString(buf.ToArray()); }
    public byte[] MemReadBytes(uint addr, int count) => _vm.GetSpan(addr, count);
    public void MemWriteBytes(uint addr, ReadOnlySpan<byte> data) => _vm.WriteBytes(addr, data);
    ...
    { var h = _nextHandle; _nextHandle += 4; _handles[h] = obj; return h; }
    public bool TryGetHandle<T>(uint handle, out T? value) where T : class
    { if (_handles.TryGetValue(handle, out var obj) && obj is T t) { value = t; return true; } value = null; return false; }
    ...
    { var init = AlignUp(dwInitialSize == 0 ? 0x10000u : dwInitialSize, 0x1000); var max = dwMaximumSize == 0 ? init : AlignUp(dwMaximumSize, 0x1000); var baseAddr = SimpleAlloc(init); _heaps[baseAddr] = new HeapState(baseAddr, baseAddr, baseAddr + max); return baseAddr; }
    ...
    public uint HeapAlloc(uint hHeap, uint dwBytes)
    {
        ...
        { var ptr = hs.Current; hs = hs with { Current = hs.Current + size }; _heaps[hHeap] = hs; return ptr; }
    ...
    { var size = AlignUp(dwSize == 0 ? 1u : dwSize, 0x1000); if (lpAddress != 0){ if (lpAddress + size <= _vm.Size) _vm.WriteBytes(lpAddress, new byte[size]); return lpAddress; } var addr = AlignUp(_allocPtr, 0x1000); _allocPtr = addr + size; _vm.WriteBytes(addr, new byte[size]); return addr; }
