...
namespace Win32Emu.NeParser
{
	...
	private static List<string> ParseImportModuleTable(byte[] bytes, NeHeader header)
	{
		...
		// Standard interpretation: offsets are relative to the start of the Imported Names Table.
		// However, some NE files (particularly older ones) may use offsets relative to the NE header base.
	...
	private static List<string> TryParseModuleNames(byte[] bytes, NeHeader header, int moduleTableOffset, int baseOffset, ushort moduleCount)
	{
		...
		for (var i = 0; i < moduleCount; i++)
		{
			...
			// Calculate actual offset based on interpretation
			var actualOffset = baseOffset + nameOffset;
			...
			// Quick validation: skip if the name contains null bytes or non-printable characters
