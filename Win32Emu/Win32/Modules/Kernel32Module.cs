	...
	public class Kernel32Module : IWin32ModuleUnsafe
	{
		...
		public unsafe bool TryInvokeUnsafe(string exp, ICpu cpu, VirtualMemory memory, out uint returnValue)
		{
			...
			switch (exp.ToUpperInvariant())
			{
				...
				default:
					_logger.LogInformation($"[Kernel32] Unimplemented export: {exp}");
					return false;
			}
		}

		[DllModuleExport(489, entryPoint: 0x000233FD, version: "winme")]
		[DllModuleExport(478, entryPoint: 0x00011752, version: "winxp")]
		...
		[DllModuleExport(7)]
		private uint GetAcp() => 1252; // Windows-1252 (Western European)

		[DllModuleExport(9)]
		private uint GetCpInfo(uint codePage, uint lpCpInfo)
		{
			if (lpCpInfo == 0)
			{
		...
		[DllModuleExport(18)]
		private uint GetProcAddress(uint hModule, uint lpProcName)
		{
			...
			if (byOrdinal)
			{
				...
				var exportOrdinals = DllModuleExportInfo.GetAllExports(emulatedModule.Name);
		...
		[DllModuleExport(8)]
		private uint GetCommandLineA() => _env.CommandLinePtr;
